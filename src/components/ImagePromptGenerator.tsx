...
Respond with a single short paragraph.`;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const responses = await Promise.all([
        generatePromptWithAPI(inputTitle, true, apiKey),
        generatePromptWithAPI(inputTitle, false, apiKey)
      ]);

      setPrompt(responses[0]);
      setPromptWithoutTitle(responses[1]);
  ...
  const generatePromptWithAPI = async (title: string, includeTitle: boolean, apiKey: string) => {
    const promptTemplate = generatePromptTemplate(title, includeTitle);
  ...
  return (
    <div className="space-y-6">
      <ModelLet's add a new output field to display the prompt without the title text requirement.

<lov-code>
Let's update the ImagePromptGenerator.tsx file to include a new output field:

<lov-write file_path="src/components/ImagePromptGenerator.tsx">
import { useState } from "react";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
