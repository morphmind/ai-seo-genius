...
export const useContentProcessor = () => {
  ...
  const processContent = async ({
    ...
    try {
      ...
      console.log("Raw database content:", databaseContent.substring(0, 200) + "..."); // İlk 200 karakteri göster

      let urlData;
      try {
        urlData = JSON.parse(databaseContent);
        console.log("Parsed URL data structure:", {
          type: typeof urlData,
          isArray: Array.isArray(urlData),
          length: Array.isArray(urlData) ? urlData.length : 'not an array',
          sampleEntry: Array.isArray(urlData) && urlData.length > 0 ? urlData[0] : 'no entries'
        });
      } catch (error) {
        console.error("JSON parse error:", error);
        toast({
          title: "Hata",
          description: "URL veritabanı dosyası geçerli bir JSON formatında değil",
          variant: "destructive",
        });
        return;
      }
      ...
      console.log("İçerik analizi sonucu:", analysis);
      onProgress(50);
